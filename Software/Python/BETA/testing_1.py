...
def setup_windows():
    ...
    cv2.namedWindow( "blob" )                                                  # Start a named window for output
    cv2.createTrackbar( "Black = 0 White = 255"  , "blob", 255, 255, placeholder )
    cv2.createTrackbar( "minRadius"              , "blob", 15 , 100, placeholder )      # Trackbars for blob detector
    cv2.createTrackbar( "maxRadius"              , "blob", 40 , 500, placeholder )      # parameters.
    cv2.createTrackbar( "Circularity"            , "blob", 26 , 100, placeholder )#40   #     (used in find_pupil)
    cv2.createTrackbar( "Convexity"              , "blob", 43 , 100, placeholder )#15   # ...
    cv2.createTrackbar( "InertiaRatio"           , "blob", 41 , 100, placeholder )      # ...
    return()
...
def update_detector():
    ...
    BlackOrWhite = cv2.getTrackbarPos( "Black = 0 White = 255"  , "blob" )
    r_min        = cv2.getTrackbarPos( "minRadius"              , "blob" )                    # Get updated blob detector
    r_max        = cv2.getTrackbarPos( "maxRadius"              , "blob" )                    # parameters
    circle_min   = cv2.getTrackbarPos( "Circularity"            , "blob" )                    # ...
    convex_min   = cv2.getTrackbarPos( "Convexity"              , "blob" )                    # ...
    inertia_min  = cv2.getTrackbarPos( "InertiaRatio"           , "blob" )                    # ...
...
def procFrame( image ):                                                             # procceses image so it is more reliable in blob detection
...
##    maxHSV = np.array([138, 255, 37])
    ...
    return( processed )                                                             # returns processed image
...
while(stream.isOpened()):
    ...
    if ret == True:
        ...
        image, img_detected = find_pupil( proc, overlay, overlayImg, frame )
